...
const ChatThreadScreen = ({ navigation, route }: any) => {
  ...
  const handleSendMessage = async (content: string, medias: any[] = []) => {
    ...
    dispatch({
      ...
      payload:
        chatType === 'channel' ? !state.channelCallback : !state.callback,
    });
    dispatch({ type: ACTIONS.MEDIA, payload: [] });
    clearUploads();
  };
  ...
  const dmReferenceLabel =
    headerParticipant?.username ||
    headerParticipant?.full_name ||
    'Direct message';
  ...
  return (
    <Container color={colors.topNavigation}>
      ...
      <ThreadScreenHeader
        ...
        referenceType="user"
        referenceLabel={dmReferenceLabel}
        referenceAvatarUrl={
          headerParticipant?.avatar_url || headerParticipant?.default_avatar_url
        }
      />
